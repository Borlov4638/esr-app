...
class Config:
    def __init__(self):
        ...
        self.time = os.environ.get('PITCH_TIME_STEP') or float(input('Шаг для частот:'))
        self.floor = os.environ.get('FLOOR_F0') or float(input('Нижняя граница ЧОТ:'))
        self.ceiling = os.environ.get('CEILING_F0') or float(input('Верхняя граница ЧОТ:'))
        self.step_per = os.environ.get('PERCENTAGE_STEP') or int(input('Шаг для процентов:'))
        self.step_pat = os.environ.get('PATTERN_STEP') or int(input('Шаг для паттернов:'))
